...
def demodulate(id, globalInfo, start, end):
    ...
    for i in range(fix_start, end):
        ...
        img_Sepcular = cv.imread(os.path.join(globalInfo.mPath, globalInfo.PrefixSpe + idx + ".exr"), cv.IMREAD_UNCHANGED)
        img_Metallic = cv.imread(os.path.join(globalInfo.mPath, globalInfo.PrefixMetallic + idx + ".exr"), cv.IMREAD_UNCHANGED)
